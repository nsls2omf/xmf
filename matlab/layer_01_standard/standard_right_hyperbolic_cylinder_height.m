function [z_quad_sln, z_expression] = standard_right_hyperbolic_cylinder_height(x, abs_p, abs_q, theta)
...
if abs_p > abs_q % right and p>q, means convex hyperbolic cylinder
    z_quad_sln = (-B + sqrt(Delta))./(2.*A);
else % right and p<q, means concave hyperbolic cylinder
    z_quad_sln = (-B - sqrt(Delta))./(2.*A);
end
