function [z2d_quad_sln, z2d_expression] = standard_right_hyperboloid_height(x2d, y2d, abs_p, abs_q, theta)
...
if abs_p > abs_q % p>q and right means convex hyperboloid
    z2d_quad_sln = (-B + sqrt(Delta))./(2.*A);
else % p<q and right means concave hyperboloid
    z2d_quad_sln = (-B - sqrt(Delta))./(2.*A);
end
